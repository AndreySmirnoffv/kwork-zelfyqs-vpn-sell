...
export async function succeededSubpscriptionPayment(bot, userId, paymentId, type) {
    try {
        ...
        if (status === "succeeded") {
            ...
            if (!user) {
                throw new Error(`Пользователь с chatId ${userId} не найден`);
            }
            ...
            return await bot.sendMessage(userId, `Вы успешно продлили подписку до ${endDate.toLocaleDateString()}`);
        }
    } catch (error) {
